...
public class SwordTotemScript : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.GetComponent<Collider2D>().tag == "attack" && !casted)
        {
            ...
            controller.GetComponent<GeneralGameController>().currentPlayerDamage = controller.GetComponent<GeneralGameController>().currentPlayerDamage / 2;
